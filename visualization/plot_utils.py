...
def render_simulator_path_summary_png(path_type, params, display_path_label=None):
    """
    Matplotlib PNG for the web UI: scaled axes (m), trajectory, observer,
    path length L, clip duration T, speed v, and the relation T = L/v for
    constant-speed motion along the path (same as map / custom-path physics).
    ...
    # Figure: plot uses full width; metrics live in a dedicated strip below (no overlap with data).
    ...
    metrics_text = (
        f'·  {label}  ·\n\n'
        rf'·  $L={Ls}\,\mathrm{{m}}$ · $v={vs}\,\mathrm{{m/s}}$ · duration $T=L/v={Ts}\,\mathrm{{s}}$  ·'
    )
